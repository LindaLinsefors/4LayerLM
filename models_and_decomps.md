...
| **F** | `p-c45e0001` | sink seed 45 (`t-87f91319`) **with the CORRECTED (fitted) RoPE forward** | 100k | 36,864 | 17,722 (sample mean CI > 1e-6, fitted-RoPE forward; `coci-heatmaps/hide/cache/coci_F.npz`) | our re-run of C's exact recipe with the true spectrum installed — the "decomposition of the real sink LM" C should have been; trained by us on Modal, **completed 2026-09-19** (`sink-models/redo-decomps/`; substrate deviations documented in `config_ropefix_C.yaml`'s header). Final evals vs C: kl_ci_masked 0.369 (C 0.618), ce_difference_ci_masked **+0.226** (C's impossible −0.314 gone — healthy target), L0 209 (C 182), PGD recon 0.626 (C 0.847) | volume `vpd-4layer`: `/sink-models/runs/p-c45e0001/` (final ckpt `ckpts/100000`) |
| **G** | `p-47de0001` | **freqtest47** (`t-freqtest47`) with its EXACT learned RoPE spectrum installed (raw values incl. the 6 slightly-negative unconstrained planes) | 100k | 36,864 | — (in training since 2026-09-19) | C's exact recipe applied to our own fully-known model — the first decomposition in the project with a completely documented target; config `sink-models/redo-decomps/hide/config_G_freqtest47.yaml`. Target resolves via the hex-alias cache `pretrain_cache/spd-t-f4e40047/` (= a copy of `spd-t-freqtest47/`; the config parser only accepts `x-<8hex>` run ids) | volume `vpd-4layer`: `/sink-models/runs/p-47de0001/` |
