...
> **⚠ ADDENDUM 2026-09-18 — the "consequence" paragraph above is superseded for decomposition work; see [rope_decomposition_check.md](rope_decomposition_check.md).** The C/D/E decompositions themselves (training forward, logged evals, and the shipped harvests) turn out to have been trained with the *textbook base-10000* forward — the same one the public loader builds — so the public-loader CI caches are internally *consistent* with the decompositions, and it is the fitted-spectrum forward that mismatches them (the decomposition loses its sparsity there). The base-model conclusions of this report are unchanged and were re-verified on fresh data; C/D/E remain decompositions of the mis-loaded (~8-NLL) model rather than of the real sink LMs.
