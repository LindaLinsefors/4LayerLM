"""Modal GPU job: cross-site co-CI statistics for the SIGNED read-in-cosine
clusters of new A and sink decomposition C (see cluster_ci_modal.py for the
original unsigned newA job; same statistics, different member sets).

Members: every component that appears in a pair with cos(V) > 0.7 (the
cluster-chaining threshold) OR cos(V) < -0.4 (so the negative-tail pairs get
co-CI values too), from cache/vcos_signed_{decomp}.npz.  Extracts their
...
sink-loader PR #1002 image (commit 82a67f71c; the pinned public main cannot
load untied-head/attention-sink targets) against the shared /data/sink-models
root; newA runs on the pinned-commit image against its per-archive root.
Both rely on archives already on volume vpd-4layer (from
coci-heatmaps/hide/{coci_compute_new_modal.py, coci_compute_sink_modal.py})
and /pile_rows.npy.

Run (from the project root; Modal needs UTF-8 on this machine):
  PYTHONUTF8=1 PYTHONIOENCODING=utf-8 modal run cos-sim/read-in/hide/cluster_ci_signed_modal.py --decomp C
  PYTHONUTF8=1 PYTHONIOENCODING=utf-8 modal run cos-sim/read-in/hide/cluster_ci_signed_modal.py --decomp newA
"""

import modal

app = modal.App("readin-cluster-ci-signed")
...
def main(decomp: str = "C") -> None:
    ...
    cfg = DECOMPS[decomp]
    z = np.load(here / "cache" / f"vcos_signed_{decomp}.npz")
    P_ = z["pairs_raw"]
    m = (P_[:, 2] > 0.7) | (P_[:, 2] < -0.4)
