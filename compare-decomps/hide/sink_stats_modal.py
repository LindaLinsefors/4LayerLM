"""Modal GPU jobs: per-decomposition statistics for the three attention-sink
decompositions C = p-d60af588, D = p-fecd6a6b, E = p-bd411e35 (see CLAUDE.md
"Attention-sink models & decompositions") — everything the compare-decomps
widget needs per decomposition, mirroring the newA/newB caches:

  coci       -> /coci_{C,D,E}.npz        (per-matrix co-CI r f16, mean, F, T)
  top_tokens -> /top_tokens_{C,D,E}.npz  (top-20 input tokens by summed CI)
  act_signs  -> /act_signs_{C,D,E}.npz   (activation-sign stats for the
                                          signed-cos gauge, CI > 0.1 tokens)
  uv         -> /uv_{C,D,E}.npz          (U/V factors f16 — ALL components,
...
archives already extracted on the volume by sink-models/hide/
pos_fires_sink_modal.py's fetch stage.

Run:  modal run compare-decomps/hide/sink_stats_modal.py
...
def _load(name):
    ...
    return run, open_jax_run(root / "runs" / run, step=STEP, data_root=root)
...
def main() -> None:
    ...
    with cf.ThreadPoolExecutor(12) as ex:
        futs = [ex.submit(fn.remote, n)
                for fn in (coci, top_tokens, act_signs, uv) for n in names]
        for f in futs:
            f.result()
