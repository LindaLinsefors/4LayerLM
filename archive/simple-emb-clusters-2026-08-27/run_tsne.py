"""t-SNE of the SimpleStories token embeddings.

Four runs (perplexity 5/30/100 euclidean + perplexity 30 cosine), cached to
cache/tsne_*.npy. Figures:
  figures/tsne_grid.png       -- all four runs, colored by token class
  figures/tsne_annotated.png  -- main run, class colors + token labels
  figures/tsne_freq_norm.png  -- main run, colored by log-frequency and by norm
"""
...
from common import (CACHE, CLASS_COLORS, CLASS_NAMES, INK, INK_2, MUTED,
...
def tsne_coords(emb: np.ndarray, name: str) -> np.ndarray:
    f = CACHE / f"tsne_{name}.npy"
    if f.exists():
        return np.load(f)
    print(f"running t-SNE {name} ...", flush=True)
...
def scatter_classes(ax, xy, classes, s=3.0):
    for c, (cname, color) in enumerate(zip(CLASS_NAMES, CLASS_COLORS)):
        m = classes == c
        if not m.any():  # e.g. "Capitalized": the tokenizer lowercases everything
            continue
...
def main():
    ...
    coords = {name: tsne_coords(emb, name) for name in RUNS}
    ...
    for i, name in enumerate(RUNS):
        ...
        style_ax(ax)
        scatter_classes(ax, coords[name], classes)
    ...
    for t in leg.get_texts():
        t.set_color(INK_2)
    fig.suptitle("t-SNE of SimpleStories token embeddings (4019 x 192, wte = lm_head)",
                 fontsize=11, color=INK)
    fig.tight_layout(rect=(0, 0.02, 1, 0.99))
    save_fig(fig, "tsne_grid.png")
    plt.close(fig)
    ...
    style_ax(ax)
    scatter_classes(ax, xy, classes, s=5)
    ...
    candidates = [tokens.index("[EOS]"), tokens.index("[UNK]")] + order[:400].tolist()
    ...
    for i in candidates:
        if placed and np.min(np.linalg.norm(np.array(placed) - xy[i], axis=1)) < min_dist:
            continue
        placed.append(xy[i])
        ax.annotate(tokens[i], xy[i], fontsize=6, color=INK,
                    xytext=(2, 2), textcoords="offset points")
        n_labels += 1
    print(f"placed {n_labels} token labels")
    ax.set_title(f"t-SNE ({titles[MAIN]}) — labels: most frequent tokens",
                 fontsize=12, color=INK)
    save_fig(fig, "tsne_annotated.png")
    plt.close(fig)

    # --- main run colored by log-frequency and by embedding norm ---------------
    fig = new_fig(figsize=(12.5, 6))
    for j, (values, label) in enumerate([
        (np.log10(1 + freq), "log10(1 + corpus count)"),
        (np.linalg.norm(emb, axis=1), "embedding row norm"),
    ]):
        ...
        ax.set_xticklabels([]); ax.set_yticklabels([])
    fig.suptitle(f"t-SNE ({titles[MAIN]})", fontsize=11, color=INK)
    fig.tight_layout()
    save_fig(fig, "tsne_freq_norm.png")
    plt.close(fig)
