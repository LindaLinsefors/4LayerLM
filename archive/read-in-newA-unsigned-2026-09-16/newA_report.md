# New A: read-in (V) cosine similarity across the residual-stream-reading matrices


## Headlines

- **The high-cosine tail is token-identity machinery.** 981 of 8657
  pooled components (11%) sit in 289
  clusters at |cos(V)| > 0.7, and almost every cluster is "all the
  ...
  **between** sites, not within one matrix (457
  same-layer cross-matrix + 289 cross-layer vs
  351 same-matrix pairs above 0.7): q, k, v and c_fc
...
- **Each token has two orthogonal read directions: the embedding (read by
  ...
  |cos(V, wte)| = 0.66), upper-layer members' do not
  (medians 0.04 / 0.03 /
  0.04 for L1/L2/L3) — yet upper-layer members of
  one cluster agree with each other at 0.7+. Accordingly the |cos| > 0.7
...
| `' not'` | 2 | 2 | 0.76 | 0.11 | 0.09 |
  ...
  *class* detectors (letters, Spanish function words, Cyrillic bytes,
  ...
  12's L3c_fc:858 with the newline readers, cluster
  13's L3c_fc:2156 with the ' of' readers) — the raw
...
- **The pos-0/EOS sink machinery shares a read-in too.** The big
  ...
  0.05) — consistent with it reading the shared massive-vector /
  boundary direction of the endoftext-pos0 sub-project.
- **Aligned readers are not component duplicates.** Same-matrix pairs above
  0.7 co-fire moderately (median co-CI r = 0.61) but write
  near-orthogonally (median |cos(U)| = 0.08) — the
...
pooled into one set of **8657 components**; `o_proj` (reads attention-head
...
vectors in $d=768$: $\sqrt{2/\pi d} \approx 0.029$. Scripts:
...
12531/1097/24 raw vs
12513/1080/26 gain-folded —
...
|---|---|---|---|---|---|---|
| pairs above | 12531 | 6586 | 3046 | 1097 | 231 | 24 |

(37.5M pairs total; max |cos| = 0.949.)

Of the 1097 pairs above 0.7: **351 same-matrix,
457 same-layer cross-matrix, 289
...
with ≥ 2 members, covering 981 of 8657 components**. Clusters are
...
present in the graph (> 0.7). Below each table: member × member heatmaps of
...
![cluster 1](hide/figures/clusters/cluster_1.png)
...
![cluster 2](hide/figures/clusters/cluster_2.png)
...
![cluster 3](hide/figures/clusters/cluster_3.png)
...
![cluster 4](hide/figures/clusters/cluster_4.png)
...
![cluster 5](hide/figures/clusters/cluster_5.png)
...
![cluster 6](hide/figures/clusters/cluster_6.png)
...
![cluster 7](hide/figures/clusters/cluster_7.png)
...
![cluster 8](hide/figures/clusters/cluster_8.png)
...
![cluster 9](hide/figures/clusters/cluster_9.png)
...
![cluster 10](hide/figures/clusters/cluster_10.png)
...
![cluster 11](hide/figures/clusters/cluster_11.png)
...
![cluster 12](hide/figures/clusters/cluster_12.png)
...
![cluster 13](hide/figures/clusters/cluster_13.png)
...
![cluster 14](hide/figures/clusters/cluster_14.png)
...
![cluster 15](hide/figures/clusters/cluster_15.png)
...
![cluster 16](hide/figures/clusters/cluster_16.png)
...
![cluster 17](hide/figures/clusters/cluster_17.png)
...
![cluster 18](hide/figures/clusters/cluster_18.png)
...
![cluster 19](hide/figures/clusters/cluster_19.png)
...
![cluster 20](hide/figures/clusters/cluster_20.png)
...
![cluster 21](hide/figures/clusters/cluster_21.png)
...
![cluster 22](hide/figures/clusters/cluster_22.png)
...
![cluster 23](hide/figures/clusters/cluster_23.png)
...
![cluster 24](hide/figures/clusters/cluster_24.png)
...
![cluster 25](hide/figures/clusters/cluster_25.png)
...
![cluster 26](hide/figures/clusters/cluster_26.png)
...
![cluster 27](hide/figures/clusters/cluster_27.png)
...
![cluster 28](hide/figures/clusters/cluster_28.png)

### Cluster 29 — n = 7 (L0c_fc, L0q, L0v, L3c_fc); edge |cos| 0.70–0.82

| matrix | component | mean CI | pos-0 fires | emb align | top tokens |
|---|---|---|---|---|---|
| L0q | 342 | 6.6e-03 | 0% | 0.74 | `' to'` `'to'` `'To'` |
| L0v | 486 | 8.5e-03 | 1% | 0.70 | `' to'` `' To'` `'to'` `'To'` |
| L0c_fc | 1251 | 1.1e-02 | 0% | 0.61 | `' to'` `'to'` `' into'` `'To'` `' To'` |
|  | 649 | 7.8e-03 | 0% | 0.68 | `' to'` `'to'` `'To'` `' To'` |
|  | 858 | 5.6e-03 | 0% | 0.63 | `' to'` `'to'` `' To'` |
|  | 864 | 4.2e-03 | 0% | 0.52 | `' to'` `'to'` `' towards'` |
| L3c_fc | 917 | 3.3e-02 | 0% | 0.00 | `' used'` `' and'` `' have'` `' need'` `','` |

![cluster 29](hide/figures/clusters/cluster_29.png)

### Cluster 30 — n = 7 (L0c_fc, L0q, L0v, L3c_fc); edge |cos| 0.71–0.86

| matrix | component | mean CI | pos-0 fires | emb align | top tokens |
|---|---|---|---|---|---|
| L0q | 389 | 5.2e-03 | 0% | 0.80 | `' that'` `' which'` `' That'` `'that'` `'That'` |
| L0v | 150 | 5.7e-03 | 1% | 0.68 | `' that'` `' which'` `' who'` `' That'` `'that'` |
| L0c_fc | 1991 | 6.8e-03 | 0% | 0.76 | `' that'` `' which'` `' who'` `' That'` `' whether'` |
|  | 728 | 3.3e-03 | 0% | 0.69 | `' that'` `'that'` `' That'` |
|  | 579 | 2.8e-03 | 0% | 0.63 | `' that'` `' That'` `'That'` `'that'` |
|  | 1339 | 2.5e-03 | 1% | 0.63 | `' that'` `' That'` `'That'` `'that'` |
| L3c_fc | 230 | 1.1e-02 | 0% | 0.11 | `' and'` `' is'` `','` `' so'` `' such'` |

![cluster 30](hide/figures/clusters/cluster_30.png)

### Cluster 31 — n = 6 (L0c_fc, L0k, L0q, L0v); edge |cos| 0.71–0.86
...
![cluster 31](hide/figures/clusters/cluster_31.png)

### Cluster 32 — n = 6 (L0c_fc, L0k, L0q, L0v); edge |cos| 0.71–0.94
...
![cluster 32](hide/figures/clusters/cluster_32.png)

### Cluster 33 — n = 6 (L1c_fc, L1k, L1q, L1v); edge |cos| 0.72–0.83
...
![cluster 33](hide/figures/clusters/cluster_33.png)

### Cluster 34 — n = 6 (L2c_fc, L2k, L2q, L2v); edge |cos| 0.72–0.87
...
![cluster 34](hide/figures/clusters/cluster_34.png)

### Cluster 35 — n = 6 (L2c_fc, L2k, L2q, L2v); edge |cos| 0.75–0.88
...
![cluster 35](hide/figures/clusters/cluster_35.png)

### Cluster 36 — n = 5 (L0c_fc, L0k, L0q, L0v); edge |cos| 0.75–0.82
...
![cluster 36](hide/figures/clusters/cluster_36.png)

### Cluster 37 — n = 5 (L0c_fc, L0k, L0q, L0v); edge |cos| 0.70–0.93
...
![cluster 37](hide/figures/clusters/cluster_37.png)

### Cluster 38 — n = 5 (L0c_fc, L0k, L0q, L0v); edge |cos| 0.72–0.84
...
![cluster 38](hide/figures/clusters/cluster_38.png)

### Cluster 39 — n = 5 (L0c_fc, L0k, L0q, L0v); edge |cos| 0.83–0.93
...
![cluster 39](hide/figures/clusters/cluster_39.png)

### Cluster 40 — n = 5 (L1c_fc, L1v, L2c_fc, L2v); edge |cos| 0.71–0.84
...
![cluster 40](hide/figures/clusters/cluster_40.png)

### Cluster 41 — n = 5 (L2c_fc, L2v, L3c_fc, L3v); edge |cos| 0.71–0.75

| matrix | component | mean CI | pos-0 fires | emb align | top tokens |
|---|---|---|---|---|---|
| L2v | 424 | 1.4e-02 | 0% | 0.06 | `'v'` `'V'` `' V'` `' v'` `' value'` |
| L2c_fc | 293 | 1.8e-03 | 0% | 0.07 | `'V'` `'v'` `' V'` `' v'` `'ov'` |
| L3v | 145 | 2.8e-03 | 63% | 0.00 | `' V'` `'V'` `'v'` `'\n'` `' v'` |
| L3c_fc | 534 | 1.4e-03 | 0% | 0.02 | `' V'` `'V'` `'v'` `' v'` `'av'` |
|  | 2013 | 3.5e-04 | 0% | 0.01 | `' V'` `'V'` `' v'` `'v'` |

![cluster 41](hide/figures/clusters/cluster_41.png)

### Smaller clusters (3–4 members, 64 of them) — grouped by kind
...
| L1c_fc:799 ↔ L2k:638 | 0.92 | cross-layer | 0.54 |  | `'-'` `'.'` `'_'` / `'-'` `'.'` `'_'` |
...
write-side |cos(U)| = 0.08 (1 pairs > 0.7,
196 < 0.1). The 746 **cross-site** pairs co-fire
similarly: median co-CI r = 0.68 (quartiles
0.51–0.82; 87 > 0.9,
32 < 0.1; from the cross-site CI Gram of all clustered
