...
of h.0.attn.q_proj AND h.0.attn.k_proj of the sink decompositions D and E
(p-fecd6a6b [target t-87f91319] / p-bd411e35 [target t-75f6c439]) over the
4,000 cached Pile rows — the D/E analogue of ci_per_token_modal_C.py (sink
PR #1002 commit, shared data root /data/sink-models, step 100000).
...
Output per decomposition x matrix: cache/ci_per_token_{D,E}_<h0q|h0k>_top20.npz
("ids", "ci_sum", "fires") — summed CI and fire counts (CI > 0.1) per input
token id, (vocab, 20); ids selected locally from compare-decomps coci_{D,E}.npz.

Run:  PYTHONUTF8=1 PYTHONIOENCODING=utf-8 modal run components/hide/ci_per_token_modal_DE.py
"""

import modal

app = modal.App("components-ci-per-token-de")
...
RUNS = {"D": "p-fecd6a6b", "E": "p-bd411e35"}
STEP = 100000
VOCAB = 50277
N_TOP = 20
FIRE_THRESH = 0.1
...
def main() -> None:
    ...
    jobs = []
    for dec in ("D", "E"):
        coci = np.load(here.parents[1] / "compare-decomps" / "hide" / "cache"
                       / f"coci_{dec}.npz")
