"""RoPE-rotated q.k score heatmaps for decomposition C, h.<l>.attn, layers 0-3.
...
from cache/typical_act_C.npz (typical_act_modal_C.py, corrected-RoPE forward).
...
(p, p+64) paired) -- NOT the recorded-but-wrong rotary_base 10000
(sink-models/rope_report.md); the fitted spectrum is the C/D/E standard until
the true training convention is known.
...
Output: components/C/q_dot_k/<fixed matrix short>/<fixed CI:.2f>-CI-<id>.png
(folder names the FIXED component's matrix, like the other components/C
...
def make_figure(fixed_mod, fixed_id, vary_mod, order, out):
    ...
    fig.suptitle(
        f"C  {fixed_mod}:{fixed_id} · RoPE(D) · {vary_mod} (alive, {N} "
...
for fixed_mod, vary_mod in [(f"h.{l}.attn.{a}_proj", f"h.{l}.attn.{b}_proj")
    ...
    outdir = ROOT / "components/C/q_dot_k" / fixed_mod.replace("_proj", "")
