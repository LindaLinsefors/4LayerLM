"""Max/min-over-distance RoPE-rotated q.k heatmaps for decomposition F, h.0.attn.
...
cached Pile rows, cache/typical_act_F.npz), so signs are meaningful; unlike
...
mean CI descending (ties by id); each panel's symmetric color scale spans
that panel's own max |value| (scales not comparable across panels).

Outputs: components/F/q_dot_k_max/h.0.attn.png (all alive comps) and
...
def make_figure(Smax, Smin, oq, ok, scale, tick_step, out, subtitle):
    ...
    for r, title in enumerate(titles):
        for c, (M, rname) in enumerate([(Smax[r], "max"), (Smin[r], "min")]):
            ...
            # per-panel symmetric scale: this panel's own max |value|
            vmax = np.abs(M).max()
            im = ax.imshow(M, aspect="auto", cmap="RdBu_r", vmin=-vmax,
                           vmax=vmax, interpolation="nearest")
            ax.set_title(f"{title} — {rname} over D   (color range "
                         f"±{vmax:.3f} = this panel's |max|; panel max "
    ...
    fig.suptitle(
        f"F  h.0.attn: max/min over distance D=0..511 of "
        f"(R_D U_q)·U_k/√128 per head ({subtitle})\n"
        "unweighted; sign gauge = typical-activation sign; corrected fitted "
        "RoPE spectrum; color range per panel = that panel's own |max| "
        "(scales NOT comparable across panels); ticks = component ids",
...
outdir = ROOT / "components/F/q_dot_k_max"
