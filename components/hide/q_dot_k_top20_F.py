"""RoPE-rotated q.k score heatmaps for F, h.<l>.attn layers 0-3 -- top-20 zoom.

Same figures as q_dot_k_F.py (see its docstring for the score definition and
...
Output: components/F/q_dot_k_top20/<fixed matrix short>/<fixed CI:.2f>-CI-<id>.png
(folder names the FIXED component's matrix, standard components/F convention).
...
def make_figure(fixed_mod, fixed_id, vary_mod, order, out):
    ...
    fig.suptitle(
        f"F  {fixed_mod}:{fixed_id} · RoPE(D) · {vary_mod} (top-{N_TOP} by "
...
for fixed_mod, vary_mod in [(f"h.{l}.attn.{a}_proj", f"h.{l}.attn.{b}_proj")
    ...
    outdir = ROOT / "components/F/q_dot_k_top20" / fixed_mod.replace("_proj", "")
